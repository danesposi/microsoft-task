...
const TaskDisplayer = () => {
    ...
    return (
        <div className='bg-blue-800/90 opacity-80 rounded-tl-lg h-screen flex justify-center'>
            <div className='flex flex-col w-[85%] my-12'>
                ...
                <div className='rounded-md bg-slate-100/75 mt-2'>
                    <form className='flex items-center mx-5 ' onSubmit={handleSubmitTask}>
                        ...
                        <input maxLength={80} value={taskTitle} onChange={(evt) => setTaskTitle(evt.target.value)} className='min-w-0 pt-4 pb-4 flex-1 bg-slate-100/5 focus:outline-none placeholder-black placeholder-opacity-100' placeholder='Add task'></input>
                    </form>
                </div>             
            </div>
        </div>
